...
--------------------------
-- Une transaction permet de ne rien insérer si une erreur est intervenu pendant l'execution du script.
-- ==================== --
BEGIN;
...
BEGIN
    ...
    WITH RECURSIVE CommentTree AS ( -- Liste les posts (pas les commentaires)
        SELECT p.id::BIGINT, p.parent::BIGINT, p.content::TEXT, (1)::INTEGER AS level, p.like::INTEGER, p.share::INTEGER
        FROM "view_post" p
        WHERE p.id = post_id -- Filter for the specified post ID
      UNION ALL -- Liste les commentaire d'un post
        SELECT p.id::BIGINT, p.parent::BIGINT, p.content::TEXT, (ct.level + 1)::INTEGER, p.like::INTEGER, p.share::INTEGER
        FROM "view_post" p
